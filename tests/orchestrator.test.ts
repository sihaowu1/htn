...
test('early partial and exploratory branches beat duplicate successful form-filling routes', async () => {
  ...
  try {
    const { plan } = await orchestratePaths(map, goal, 'run', { call: async () => { throw new Error('No model needed'); } },
      new Trace(log, { runId: 'run', agentId: 'orchestrator', role: 'orchestrator' }), new AbortController().signal, dir);
...
test('goal-confirmed routes win; otherwise fallback prioritizes goal progress over detour length', async () => {
  ...
  const log = new EventLog(join(dir, 'events.jsonl'), false); await log.init();
  const trace = new Trace(log, { runId: 'run', agentId: 'orchestrator', role: 'orchestrator' });
  const model: Model = { call: async () => { throw new Error('No diversity decision needed'); } };
  try {
    const partial = await orchestratePaths(map, goal, 'run', model, trace, new AbortController().signal, dir);
    ...
    const complete = await orchestratePaths(map, goal, 'run', model, trace, new AbortController().signal, dir);
