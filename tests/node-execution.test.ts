...
test('exploratory workers follow a distinct cached branch and validate live actions and completion', async () => {
  ...
  const model: Model = { call: async (_h, _a, name, schema, _i, input: any) => {
  ...
  try {
    const result = await executeExploratoryBranch(task, branch, 'Find results', {} as any, model, harness, agent, new AbortController().signal, 5, helpers, () => {});
    assert.equal(result.status, 'succeeded'); assert.deepEqual(actions, ['#alternate', '#next']);
    at = 'root'; actions.length = 0;
    const invalid: Model = { call: async (_h, _a, _n, schema) => schema.parse({ decision: 'act', action: { kind: 'click', selector: '#invented', value: '' }, reason: 'Bad action', evidence: '' }) };
    await assert.rejects(executeExploratoryBranch(task, branch, 'Find results', {} as any, invalid, harness, agent, new AbortController().signal, 5, helpers, () => {}), /not present/);
    assert.deepEqual(actions, ['#alternate']);
    at = 'root'; actions.length = 0;
    const limited = await executeExploratoryBranch(task, branch, 'Find results', {} as any, model, harness, agent, new AbortController().signal, 1, helpers, () => {});
...
test('partial workers execute the entire frontier and never report success from a checkout URL', async () => {
  ...
  const model: Model = { call: async (_h, _a, name, schema) => {
  ...
  try {
    const result = await executeNodeSequence({ name: 'Partial', transitionIds: ['next'], instructions: goal,
      ...
      async () => ({ page: {} as any, dispose: async () => { disposed = true; } }), model, harness, agent, new AbortController().signal,
...
test('worker rejects a completion claim without actual final evidence', async () => {
  ...
  const { adapter, harness, agent } = await fixture();
  const model: Model = { call: async (_h, _a, _n, schema) => schema.parse({ satisfied: true, evidence: 'Payment entered', reason: 'Claim' }) };
  try {
    const result = await executeNodeSequence({ name: 'Claim', transitionIds: [], instructions: 'Enter payment', stopCondition: 'Payment entered' },
      map, 'Enter payment', async () => ({ page: {} as any, dispose: async () => {} }), model, harness, agent, new AbortController().signal,
...
test('selected-path worker receives and completes node instructions one at a time in one front-page session', async () => {
  ...
  try {
    ...
    const events = [...adapter.events.values()];
    assert.equal(events.filter(event => event.event_type === 'worker.node.review_disagreed').length, 2);
...
test('a completed path whose final state is not checkout is incomplete for a checkout goal', async () => {
  ...
  const { adapter, harness, agent } = await fixture();
  let state = 'root';
  const model: Model = { call: async (_harness, _agent, name, schema) => schema.parse(name === 'verify_path_goal'
