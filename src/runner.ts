        ...
        if (supplied) {
          ...
          const tree = await readOrDiscoverTree(run.targetUrl, async () => {
            ...
            try {
              ...
              await emit(crawler, timedOut ? 'discovery.timeout' : 'discovery.interrupted', { note, states: run.map.states.length, transitions: run.map.transitions.length });
            }
          ...
          run.map = tree.map;
          await emit(system, tree.source === 'cache' ? 'map.cache.loaded' : 'map.cache.saved', { file: tree.file, status: run.map.status });
          this.publish(run);
        }
