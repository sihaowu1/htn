...
export function fingerprint(snapshot: Pick<Snapshot, 'url' | 'title' | 'text' | 'elements'>): string {
  const normalized = {
    url: snapshot.url,
    title: snapshot.title,
    text: snapshot.text.slice(0, 4000),
    elements: snapshot.elements.map(element => ({
      tag: element.tag, type: element.type, selector: element.selector, label: element.label,
    })),
  };
  return createHash('sha256').update(JSON.stringify(normalized)).digest('hex').slice(0, 16);
}
export async function inspect(page: Page): Promise<Snapshot> {
  const result = await page.evaluate(script => (0, eval)(script), snapshotScript) as Snapshot;
