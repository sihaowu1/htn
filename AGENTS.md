...
- **UI entry points:** Watchtower's landing page is `public/index.html`, with CSS tower artwork and a local illustrative demo dialog. Get started opens `public/dashboard.html`; its Control Room page places Task Assignment before the Observation hub. The landing page does not start runs or connect to live services. Shared mascot styling lives in `public/styles.css`.
- **Discovery graph preview:** Dashboard runs request `previewGraph: true`. Below the lookout, `public/graph.js` always reveals `logs/tree_demo.json` states in stored `states[]` order through `/api/demo-tree`, independently of the live run map, spaced 1.1 seconds apart. The graph precomputes a top-down branching layout, centers parents over their children, keeps shared states/cycles as dashed references, and provides fit/zoom controls; node reveal order does not determine position. After discovery the runner enters `previewing`, awaiting `POST /api/runs/:id/graph-preview/complete` before orchestration or workers. The panel collapses before acknowledgement and remains available for review. Cancellation interrupts the wait; a ten-minute preview timeout fails the run without launching workers. Reconnecting dashboards reconstruct the demo graph from the same file. The endpoint sends only labels and connections, and reports missing or invalid demo files explicitly. API callers default to no preview; single-action mode skips it. Coverage and raw map details remain separate from worker outcomes.
...
- Preserve run ID, agent ID, and Browserbase session ID correlation across actions, model calls, browser events, errors, and outcomes. Instrument every agent role through the shared telemetry layer. Durable local logging must not depend on Sentry availability.
- Observer findings must cite real log events and distinguish observed failures from suspected causes. Exclude observer-generated events from its own analysis input.
...
- Start development with `npm run dev`. Build with `npm run build`; run the compiled server with `npm start`.
