...
- **Observation:** `src/database.ts` persists authoritative evidence in PostgreSQL and transactionally creates deduplicated investigation jobs. `src/observer-worker.ts` consumes jobs through PostgreSQL notifications and leases; `src/investigation.ts` investigates through the scoped read-only tools in `src/evidence-tools.ts`. When configured, `src/search.ts` maintains optional Elastic Cloud event and investigation indices through a durable PostgreSQL outbox. Search candidates are hydrated from PostgreSQL before use and Elasticsearch never becomes an evidence or citation authority.
